...
>   (响应哪些私聊)、对应黑名单 `FOX_QQ_BOT_GROUP_BLACKLIST` / `FOX_QQ_BOT_PRIVATE_BLACKLIST`,
>   以及 `FOX_QQ_BOT_ADMIN_QQ`(谁能用斜杠命令)——这是主门禁。
>   两级各有独立的模式开关 `FOX_QQ_BOT_GROUP_BLACKLIST_MODE` /
>   `FOX_QQ_BOT_PRIVATE_BLACKLIST_MODE`(未设时继承共同缺省 `FOX_QQ_BOT_BLACKLIST_MODE`),
>   决定该级两类名单的关系:false(默认)为**白名单模式**,
>   默认全都不能用,除非配置进白名单,且**黑名单大于白名单**(两边都在=拒绝);
>   true 为**黑名单模式**,默认全都能用,除非配置进黑名单,且**白名单大于黑名单**
>   (两边都在=放行,白名单即豁免名单)。管理员永远放行;
> - **群内按成员**(可选):与主门禁同一套黑白名单语义,但模式开关独立
>   (`FOX_QQ_BOT_GROUP_USER_BLACKLIST_MODE`,默认 `true`=黑名单模式)。
>   默认放行群里所有人,用 `FOX_QQ_BOT_GROUP_USER_BLACKLIST` 拉黑个别成员
>   (其消息整条忽略);设 `false` 转白名单模式,仅
>   `FOX_QQ_BOT_GROUP_ALLOWED_USERS` 内成员 + 管理员能触发。
>   黑名单模式下白名单大于黑名单(豁免),白名单模式下黑名单大于白名单。
>   两个名单的项支持 `群号:QQ号`(仅该群)、`all:QQ号` 或裸 `QQ号`(所有群)。
>   旧开关 `FOX_QQ_BOT_GROUP_ALLOW_ALL_GROUP_USERS` 仍兼容。
>
...
**撤回提醒**:收到撤回通知(群/私聊)时,上下文追加一行`<撤回提醒: 谁(qq_id@..) 撤回了..的消息 [msg_id#..]>`——不改动原消息行,AI 下次被唤醒或查历史时自然知道有人撤回了什么;群管撤别人消息时双方都注明。若 AI 引用已撤回的消息,发送侧自动降级为普通发送(去掉无效引用,内容照发)。
...
- **取概率时按渠道乘数叠加**:参与概率的热度 = 基础热度 + TK × 渠道乘数。
  每条消息渠道乘数 `FOX_QQ_BOT_GROUP_TK_MSG_MULT`(默认 0.1),定时渠道 `FOX_QQ_BOT_GROUP_TK_TIMER_MULT`(默认 0.7);设 0 = 该渠道完全不受 TK 影响。TK 与基础热度互不干扰,各自结算。
...
启动时逐项校验:cron 表达式不可解析、prompt 为空串、target 非法或未通过黑白名单准入的项
...
待处理触发队列长度 5(正在处理的不占),满了顶掉最旧的;
自发事件(proactive/keyword)在队列里去重。
可选的**共享冷却**(默认关闭):任意触发后一段时间内压制所有概率渠道。


**队列合并取件**(`FOX_QQ_BOT_QUEUE_MERGE=true`,默认关):worker 空闲取件时不再一次取一个事件,而是取完队列中所有剩余事件合成**一个回合**:以优先级最高的事件(@ > 手动唤醒 > cron > 关键词 > 主动)为主引导语,其余 @ 的消息逐条列出让 AI 一并回应(可分别引用),额外 cron 任务逐个附带,多余的自发触发(语义已被覆盖)丢弃。适合消息密集的群:三个人先后 @ 机器人,一次回合统一回应,而不是排队跑三个回合。
...
所有涉及群/私聊目标的工具**强制黑白名单准入**(与入站门同一套判定:白名单模式下
黑名单大于白名单,黑名单模式下白名单大于黑名单):目标未通过准入直接拒绝
(含 `fox_qq_get_history`,防跨群窥屏),不依赖提示词约束。
私聊目标与入站门一致:`FOX_QQ_BOT_ADMIN_QQ` 中的管理员始终允许,不必重复写进私聊白名单。
省略目标时回退到当前会话的群/人。

**拟人发送队列**:`fox_qq_send_message` 的 `content` 支持字符串数组——AI 被引导像真人
一样把话切成几条口语化短句一次性提交。内容进入**按会话隔离**的待发送队列后工具瞬间返回
(不返回消息 ID、不拖累回合进度);后台发送循环首条秒发,后续每条间隔
`SEND_INTERVAL + rand(0, SEND_JITTER)` 秒。`[bot]` 行在每条**实际发出时**记入上下文,
顺序与群里真实所见一致。发送失败重试一次,仍失败则**丢弃该会话队列剩余条目**、
通知管理员并在下回合告知 AI。AI 被唤醒时若队列未清空,先等它发完再取上下文
(封顶 `SEND_DRAIN_TIMEOUT` 秒,超时放行防卡死);`fox_qq_send_image` / `fox_qq_send_file`
发送前同样会等队列清空,防止"如图"比图先到。
...
待处理触发队列、各种冷却是秒级瞬态,不持久化(私聊消息缓冲与群聊上下文一样持久化,重启不丢未回应消息)。
...
| `FOX_QQ_BOT_ADMIN_QQ` | — | 管理员 QQ,逗号分隔;始终可私聊,任何黑白名单不限制 |
| `FOX_QQ_BOT_BLACKLIST_MODE` | false | 群级+私聊级名单模式的共同缺省:false=白名单模式(默认全拒,黑名单大于白名单);true=黑名单模式(默认全放,白名单大于黑名单) |
| `FOX_QQ_BOT_GROUP_BLACKLIST_MODE` | 继承上项 | 群级名单模式开关(单独控制群级) |
| `FOX_QQ_BOT_PRIVATE_BLACKLIST_MODE` | 继承上上项 | 私聊级名单模式开关(单独控制私聊级) |
| `FOX_QQ_BOT_GROUP_BLACKLIST` | 空 | 群黑名单,逗号分隔(优先级随群级模式) |
| `FOX_QQ_BOT_PRIVATE_BLACKLIST` | 空 | 私聊黑名单,逗号分隔(优先级随私聊级模式;管理员不受限) |
| `FOX_QQ_BOT_GROUP_USER_BLACKLIST_MODE` | true | 成员级名单模式:true=黑名单模式(默认放行,白名单大于黑名单);false=白名单模式(默认拒绝,黑名单大于白名单)。旧名 `GROUP_ALLOW_ALL_GROUP_USERS` 仍兼容 |
| `FOX_QQ_BOT_GROUP_ALLOWED_USERS` | 空 | 群成员白名单;项格式 `群号:QQ号`(仅该群)/ `all:QQ号` / 裸 `QQ号`(所有群) |
| `FOX_QQ_BOT_GROUP_USER_BLACKLIST` | 空 | 群成员黑名单(同格式),被拒成员消息整条忽略 |
...
| `FOX_QQ_BOT_QUEUE_MERGE` | false | 队列合并取件: 一次取完所有排队事件合成一个回合(多@一并回应);关=逐个取件 |
...
| `FOX_QQ_BOT_GROUP_TK_MSG_MULT` / `FOX_QQ_BOT_GROUP_TK_TIMER_MULT` | 0.1 / 0.7 | TK 对消息/定时渠道概率的乘数(0=无影响) |
| `FOX_QQ_BOT_GROUP_CUT_LINE` | 0.1 | 比例衰减共享归零阈值(TK 与累计热度 C 共用) |
| `FOX_QQ_BOT_SEND_INTERVAL` / `FOX_QQ_BOT_SEND_JITTER` | 1.0 / 2.0 | 拟人发送节奏:短句队列首条秒发,后续间隔=基础+rand(0,抖动)秒 |
| `FOX_QQ_BOT_SEND_DRAIN_TIMEOUT` | 90 | 唤醒前等待发送队列清空上限(秒),超时放行防卡死 |
| `FOX_QQ_BOT_SEND_QUEUE_MAX` | 10 | 单次发送调用最多入队条数(超出截断并提醒 AI) |
