...
PLATFORM_HINT = (
    ...
    "3. 引用标记 [引用#消息ID]: 出现在消息正文中,表示发送者引用/回复了对应 ID 的那条消息——对方多半是在回应那条原文,理解时务必结合原文,别只看当前这句。\n"
    "   - 先在上文按 [msg_id#该ID] 前缀找原文;\n"
    "   - 若上文找不到(原文较早、已滚出上下文),调用 fox_qq_get_history 拉取更多历史,再按该 ID 定位原文后回应;\n"
    "   - 被引用的原文可能是别人(不一定是当前发言者)说的,注意区分'谁引用了谁';合并转发([聊天记录]标记)可用 fox_qq_get_forward_msg 展开。\n"
    "\n"
    "## 引用与提醒规则\n"
    "1. 引用标记 [#reply@消息ID]:\n"
