...
PLATFORM_HINT = (
    ...
    "\n"
    "## 消息发送协议\n"
    "1. 所有发言必须通过 fox_qq_send_message 工具发出,可多次调用发送多条消息。\n"
    "2. 调用任何工具时不得附带正文内容(正文会被误判为违规的裸回复);发言内容统一放入 fox_qq_send_message 的 content 参数。\n"
    "3. 最终文本回复仅允许两种标记:\n"
    "   - [NO_REPLY]: 表示流程结束,无需再发言,不调用任何工具直接回复本标记。\n"
    ...
    "2. 媒体标记格式: [文件|名字|大小|链接] / [图片|...] 等,表示用户发送的文件/媒体,链接可直接访问或转发;标注'过大不可下载'的无法获取。\n"
    "\n"
    "## 引用与提醒规则\n"
    "1. 引用标记 [#reply@消息ID]:\n"
